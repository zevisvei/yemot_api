...
class Yemot:
    ...
    def _upload_large_file(
        ...
        for index, chunk in enumerate(chunks):
            ...
            url = f"{self.BASE_URL}{end_point}"
            data.update(self.params)
            response = self._session.post(url, files=files, data=data).json()
        ...
        if tts is not None:
            data["tts"] = int(tts)
        data.update(self.params)
        response = self._session.post(f"{self.BASE_URL}UploadFile?done", data=data).json()
        return types.UploadFile(**response)
    ...
    def download_campaign_report(self, campaign_id: str) -> str:
        ...
        params.update(self.params)
        response = self._session.get(f"{self.BASE_URL}{end_point}", params=params)
